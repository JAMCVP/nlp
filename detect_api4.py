...
def main():
	...
	if request.method == 'POST' and request.files['image']:
		...
		img_name = secure_filename(imag.filename)
		create_new_folder(app.config['UPLOAD_FOLDER'])
		save_path = os.path.join(app.config['UPLOAD_FOLDER'], img_name)
		saved_path = imag.save(os.path.join(app.config['UPLOAD_FOLDER'], img_name))
		print("save_path:", save_path)
		res, bound = detect(save_path)
		x1,y1,x2,y2 = bound[0],bound[1],bound[2],bound[3]
		#cv2.imshow("2", img1)
		path = os.path.join(PROJECT_HOME,"uploads",img_name)
		#image = cv2.imread(path)
		print(path)
		npimg = np.fromstring(img1, numpy.uint8)
		...
		print(path1)
		#cv2.imshow("2", image)
		Cropped = img[int(y2):int(y1),int(x2):int(x1)]
		print(Cropped.shape)
